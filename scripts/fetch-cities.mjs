...
async function fetchCities() {
  const url = 'https://geo.api.gouv.fr/departements/31/communes?fields=nom,code,population,codesPostaux,centre';
  
  console.log('Fetching Haute-Garonne (31) communes from Geo API...');
  try {
    ...
    // Process and filter communes in Haute-Garonne (31) with population >= 2000
    const processed = data
      .map(c => {
        let lat = 43.604652; // Toulouse lat
        let lng = 1.444209;  // Toulouse lng
